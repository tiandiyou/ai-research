...
class VideoQualityChecker {
  ...
  checkHook(hook) {
    ...
    // 痛点词
    if (/焦虑|担心|困惑|迷茫|难|痛|怕|慌|愁/.test(text)) s += 25;
    // 悬念词
    if (/居然|竟然|没想到|绝对|想不到|没想到/.test(text)) s += 25;
    // 数字
    if (/\d+%|\d+万|\d+亿|\d+倍/.test(text)) s += 25;
    // 反常识/否定
    if (/不是|别再|千万别|不要|真相|竟然/.test(text)) s += 25;
  ...
  check(prd) {
    ...
    return {
      ...
      details: {
        hook: `${scores.hook >= 80 ? '✅' : '❌'} (${scores.hook})`,
        value: `${scores.value >= 70 ? '✅' : '❌'} (${scores.value})`,
        golden: `${scores.golden >= 60 ? '✅' : '❌'} (${scores.golden})`,
        emotion: `${scores.emotion >= 50 ? '✅' : '❌'} (${scores.emotion})`,
        cta: `${scores.cta >= 50 ? '✅' : '❌'} (${scores.cta})`,
      }
    };
  }
}
...
for (const file of files) {
  ...
  console.log(`📄 ${file}: ${result.total} ${result.passed ? '✅' : '❌'}`);
  console.log(`   钩子: ${result.details.hook} | 干货: ${result.details.value} | 金句: ${result.details.golden}`);
  console.log(`   共鸣: ${result.details.emotion} | CTA: ${result.details.cta}\n`);
