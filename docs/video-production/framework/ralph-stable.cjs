...
.container{text-align:center;padding:60px;position:relative}
.icon{font-size:160px;margin-bottom:50px;animation:bounce 2s infinite}
...
.gradient-bg{position:absolute;inset:0;background:radial-gradient(circle at 30%70%,rgba(255,255,255,0.1)0,transparent 50%)}
@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-20px)}}
</style></head><body><div class="gradient-bg"></div><div class="container"><div class="icon">🔗</div><div class="title">${t}</div></div></body></html>`,
...
.container{text-align:center;padding:60px;max-width:900px;position:relative}
.tag{position:absolute;top:40px;right:40px;font-size:24px;background:rgba(255,255,255,0.25);padding:12px 24px;border-radius:25px}
...
.container{text-align:center;padding:70px;border:6px solid #fff;border-radius:35px;background:rgba(0,0,0,0.2);position:relative}
.container::before{content:'';position:absolute;inset:-10px;border-radius:45px;background:linear-gradient(45deg,#4facfe,#00f2fe);z-index:-1;animation:spin 3s linear infinite}
...
.container{text-align:center;padding:60px;border:4px solid #00D4FF;border-radius:35px;background:rgba(0,212,255,0.15);position:relative}
.container::before{content:'';position:absolute;inset:-5px;border-radius:40px;background:linear-gradient(90deg,#00D4FF,#00FF88);z-index:-1;animation:pulse 2s infinite}
...
async function run() {
  ...
  console.log("╔════════════════════════════════════════╗");
  console.log("║   Ralph v4.0 - 稳定版 (无API依赖)      ║");
  console.log("╚════════════════════════════════════════╝\n");
  ...
  console.log(`帧数: ${totalFrames}帧\n`);
  
  // 3. 渲染帧
  console.log("[3/5] 渲染帧 (精美模板)...");
  ...
  console.log(`  状态: ${videoDuration > 180 && stats.size > 3*1024*1024 ? "✅ 通过" : "⚠️ 注意"}`);
  ...
  execSync("mkdir -p out && cp final-video.mp4 out/ralph-stable.mp4");
  console.log("\n✅ 完成！");
}
