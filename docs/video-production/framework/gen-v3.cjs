...
const segments = [
  ...
  { text: "Claude Code、Cursor、v0、Devin...每一个都是革命性工具！GitHub Copilot企业版代码生成率超过50%！", type: "干货" },
  { text: "AI编程效率提升！简单Web页面4小时→10分钟，提升24倍！", type: "数据" },
  { text: "REST API开发8小时→30分钟，Bug修复2小时→5分钟，全是24倍！", type: "数据" },
  { text: "真实案例！某公司裁掉50%基础程序员，效率反而提升！", type: "案例" },
  { text: "一个初级程序员+AI工具，效率超过5个高级程序员！", type: "案例" },
  { text: "Claude Code：端到端开发，自主决策，代码审查！", type: "工具" },
  { text: "Cursor：智能代码补全，预测你接下来要写什么！", type: "工具" },
  { text: "v0：前端生成神器！Devin：第一个AI软件工程师！", type: "工具" },
  { text: "正在消失的岗位：CRUD开发、简单页面、基础测试！", type: "干货" },
  { text: "不容易被替代：系统架构师、AI工程师、安全工程师！", type: "干货" },
  { text: "核心能力：复杂问题分析、系统设计、创新方案！", type: "干货" },
...
async function run() {
  console.log("╔════════════════════════════════════════╗");
  console.log("║   爆款视频生成器 - 5分钟版 v3          ║");
  console.log("║   修复：语速+视觉质量                  ║");
  console.log("╚════════════════════════════════════════╝\n");
  
  const fps = 2;
  const secPerSeg = 12;
  const totalDuration = segments.length * secPerSeg;
  
  console.log(`目标: ${segments.length}段 × ${secPerSeg}秒 = ${totalDuration}秒 (${(totalDuration/60).toFixed(1)}分钟)\n`);
  ...
  const framePerSeg = secPerSeg * fps;
  let frameIdx = 0;
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    for (let f = 0; f < framePerSeg; f++) {
      await page.screenshot({ path: `frame-${frameIdx}.png`, fullPage: true });
      frameIdx++;
    }
    await page.close();
    console.log(`  ${seg.type}: ${framePerSeg}帧`);
  }
  await browser.close();
  ...
  execSync(`ffmpeg -y -framerate ${fps} -i "frame-%d.png" -i combined-audio.mp3 -c:v libx264 -pix_fmt yuv420p -vf "scale=1080:1920" -c:a aac -shortest final-video.mp4`, { stdio: 'inherit' });
  ...
  execSync("mkdir -p out && cp final-video.mp4 out/ai-coding-v3.mp4");
  console.log("\n✅ 完成！");
}
